...
//Helpers umum untuk halaman login/profil
...
(function () {
    ...
    function updateTimeSlots() {
        ...
        // Simulasi slot yang sudah dipesan (random)
    ...
    function calculatePrice(slot, isNight) {
        ...
        let base = booking.fieldPrice;
        let surcharge = 0;

        if (isNightTime) {
            surcharge = Math.round(base * 0.25); // 25% surcharge malam
        }

        booking.totalPrice = base + surcharge;

        document.getElementById('base-price').textContent = 'Rp ' + base.toLocaleString('id-ID');
        document.getElementById('night-surcharge').textContent = '+Rp ' + surcharge.toLocaleString('id-ID');
        document.getElementById('total-price').textContent = 'Rp ' + booking.totalPrice.toLocaleString('id-ID');
...
(function () {
    ...
    // ======== Handle Forgot Password - Kirim Email ========
